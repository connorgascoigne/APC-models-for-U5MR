...
ggsave(apcRidge, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apcValidationRidge.png'), height = 10, width = 10)
ggsave(apRidge, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apValidationRidge.png'), height = 10, width = 10)
ggsave(acRidge, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/acValidationRidge.png'), height = 10, width = 10)
...
# ggsave(apcCPOridge, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apcCPOridge.png'), height = 10, width = 10)
# ggsave(apCPOridge, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apCPOridge.png'), height = 10, width = 10)
# ggsave(acCPOridge, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/acCPOridge.png'), height = 10, width = 10)
...
ggsave(apcMedianMap, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apcValidationMedianMap.png'), height = 7, width = 7)
ggsave(apcWidthMap, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apcValidationWidthMap.png'), height = 7, width = 7)

ggsave(apMedianMap, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apValidationMedianMap.png'), height = 7, width = 7)
ggsave(apWidthMap, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/apValidationWidthMap.png'), height = 7, width = 7)

ggsave(acMedianMap, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/acValidationMedianMap.png'), height = 7, width = 7)
ggsave(acWidthMap, file = paste0(kenyaPath, 'Code/Results/Validation/Plots/acValidationWidthMap.png'), height = 7, width = 7)
...
dir2014dat <-
  ...
  summarise(y = sum(died),
            N = n()) %>%
  ungroup() %>%
  mutate(region = str_to_title(region),
         outlier = ifelse(is_outlier(y), region, NA)) %>%
  arrange(y)

boxplot2014 <-
  ggplot(dir2014dat, aes(y = y, x = year)) +
  ggplot2::geom_boxplot(width=.5) +
  geom_text(aes(label = outlier), na.rm = TRUE, position = position_jitter(w = .2, h = 0)) +
  labs(y = 'Number of Deaths') +
  my.theme(axis.title.x=element_blank(),
           axis.text.x=element_blank(),
           axis.ticks.x=element_blank())
# boxplot2014

# print(xtable::xtable(dir2014dat %>% select(-year), type = 'latex'), include.rownames = FALSE)

### organising 'truth' ----

apcTrue <-
  apcSubnatFinal %>%
  filter(period == last_year) %>%
  mutate(true = median) %>%
...
apcFinalRes <-
  left_join(apcRes, dirTrue, by = c('region', 'period')) %>% 
  relocate(c('mean', 'variance', 'lower', 'median', 'upper', 'width', 'true'), .before = 'theta:1')

apFinalRes <-
  left_join(apRes, dirTrue, by = c('region', 'period')) %>% 
  relocate(c('mean', 'variance', 'lower', 'median', 'upper', 'width', 'true'), .before = 'theta:1')

acFinalRes <-
  left_join(acRes, dirTrue, by = c('region', 'period')) %>% 
  relocate(c('mean', 'variance', 'lower', 'median', 'upper', 'width', 'true'), .before = 'theta:1')

apcScores <-
  apcFinalRes %>%
  # relative bias for each posterior value
  mutate(logitMSE = (SUMMER::logit(median) - SUMMER::logit(true))^2,
         bias = (median - true),
         RMSE = (median - true)^2,
         model = 'APC') %>%
  drop_na() %>%
  group_by(model) %>%
  summarise(logitMSE = mean(logitMSE),
            bias = mean(bias),
            RMSE = sqrt(mean(RMSE)))
...
allScores <- 
  rbind(apcScores, apScores, acScores) %>%
  mutate(logitMSE = logitMSE * 10,
         bias = bias * 1000,
         RMSE = RMSE * 100)

print(xtable::xtable(allScores, type = 'latex'), include.rownames = FALSE, digits = 2)

### vs apc actual estimates ----

apcFinalRes <-
  left_join(apcRes, apcTrue, by = c('region', 'period')) %>%
  relocate(c('mean', 'variance', 'lower', 'median', 'upper', 'width', 'true'), .before = 'theta:1')

apFinalRes <-
  left_join(apRes, apcTrue, by = c('region', 'period')) %>%
  relocate(c('mean', 'variance', 'lower', 'median', 'upper', 'width', 'true'), .before = 'theta:1')

acFinalRes <-
  left_join(acRes, apcTrue, by = c('region', 'period')) %>%
  relocate(c('mean', 'variance', 'lower', 'median', 'upper', 'width', 'true'), .before = 'theta:1')
...
dirTemp <-
  ...
  mutate(period = as.numeric(years),
         directLogitEstimate = logit.est,
         directVariance = var.est) %>%
  select(region, period, directLogitEstimate, directVariance)
...
apcCPOTemp <-
  ...
  left_join(., dirTemp, by = c('region', 'period')) %>%
  # theta here are place holders
  select(region, period, predMean, directLogitEstimate, directVariance, starts_with('theta:')) %>%
  # replace the NAs with 0
  mutate(directLogitEstimate = replace_na(directLogitEstimate, 0),
         directVariance = replace_na(directVariance, 0),
         across(starts_with('theta:'),~ NA))
...
for(i in 1:length(region_list)){
  ...
  apcCPOTemp[i,6:ncol(apcCPOTemp)] <- 
    rnorm(n = nsamples, 
          mean = SUMMER::logit(apcCPOTemp$predMean[i])+apcCPOTemp$directLogitEstimate[i], 
          sd = sqrt(apcCPOTemp$directVariance[i]))
...
apcCPO <-
  ...
  mutate(select(., starts_with('theta:')) %>%
           ...
           do.call(rbind, .),
         width = upper - lower) %>%
  relocate(c('mean', 'lower', 'median', 'upper', 'width'), .before = 'theta:1')
...
for(i in 1:length(region_list)){
  ...
  apCPOTemp[i,6:ncol(apCPOTemp)] <- 
    rnorm(n = nsamples, 
          mean =  SUMMER::logit(apCPOTemp$predMean[i])+apCPOTemp$directLogitEstimate[i], 
          sd = sqrt(apCPOTemp$directVariance[i]))
...
for(i in 1:length(region_list)){
  ...
  acCPOTemp[i,6:ncol(acCPOTemp)] <- 
    rnorm(n = nsamples, 
          mean =  SUMMER::logit(acCPOTemp$predMean[i])+acCPOTemp$directLogitEstimate[i], 
          sd = sqrt(acCPOTemp$directVariance[i]))
...
allCPO <- 
  data.frame(model = c('APC', 'AP', 'AC'),
             CPO = c(sum(log(apcCPO$mean)), sum(log(apCPO$mean)), sum(log(acCPO$mean)))) %>%
  mutate(CPO = round(CPO, 2))